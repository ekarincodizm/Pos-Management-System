...
{
    public partial class ProductSelfForm : Form
    {
        public ProductSelfForm()
        {
            InitializeComponent();
        }

        private void ProducthelfForm_Load(object sender, EventArgs e)
        {
            dataGridView1.Rows.Clear();
            dataGridView1.Refresh();
            var data = Singleton.SingletonShlf.Instance().Shelfs;
        ...
        private void dataGridView1_SelectionChanged(object sender, EventArgs e)
        {
            ...
            bool enable = Singleton.SingletonShlf.Instance().Shelfs.SingleOrDefault(w => w.Id == id).Enable;
            if (enable)
            {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            using (SSLsEntities db = new SSLsEntities())
            {
                ...
                {
                    // edit
                    var obj = db.Shelf.SingleOrDefault(w => w.Id == _Id);
                    obj.UpdateDate = DateTime.Now;
                    obj.UpdateBy = Singleton.SingletonAuthen.Instance().Id;
                    obj.Code = textBoxCode.Text;
                    obj.Name = textBoxName.Text;
