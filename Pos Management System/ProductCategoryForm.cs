...
namespace Pos_Management_System
{
    public partial class ProductCategoryForm : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            using (SSLsEntities db = new SSLsEntities())
            {
                if (_Id == 0)
                {
                    ...
                    db.ProductCategory.Add(obj);
                }
                else
                {
                    // edit
                    var obj = db.ProductCategory.SingleOrDefault(w => w.Id == _Id);
                    obj.UpdateDate = DateTime.Now;
                    obj.UpdateBy = Singleton.SingletonAuthen.Instance().Id;
                    obj.Code = textBoxCode.Text;
                    obj.Name = textBoxName.Text;
        ...
        private void dataGridView1_SelectionChanged(object sender, EventArgs e)
        {
            ...
            //bool enable = Singleton.SingletonProductCategory.Instance().ProductCategories.SingleOrDefault(w => w.Id == id).Enable;
            //if (enable)
            //{
                radioButton1.Checked = true;
            //}
        ...
        public void dataGrid()
        {
            ...
            var data = Singleton.SingletonProductCategory.Instance().ProductCategories;
